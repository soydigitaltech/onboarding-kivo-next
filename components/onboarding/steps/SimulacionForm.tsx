...
const completeAndAdvance = useOnboardingStore((state) => {
 ...
 const aplicaLimiteJoven =
  edad >= 18 && edad <= 25;

 const montoMaximoJoven = 20000;

 const montoMaximoSlider = aplicaLimiteJoven
  ? montoMaximoJoven
  : REGLAS_SIMULACION.montoMaximo;

 const plazoMaximoPermitido = aplicaLimiteJoven ? 24 : 72;
 ...
 const montoInicialNormalizado = normalizarMonto(
  simulacionGuardada?.monto ?? 15000,
 );

 const montoInicial = aplicaLimiteJoven
  ? Math.min(montoMaximoJoven, montoInicialNormalizado)
...
const plazosDisponibles = useMemo(() => {
 return REGLAS_SIMULACION.plazosMeses.filter((plazo) => {
  ...
  if (monto <= 20000) {
   return plazo >= 6 && plazo <= 24;
  }

  if (monto <= 35000) {
   return plazo >= 6 && plazo <= 36;
  }

  if (monto <= 50000) {
   return plazo >= 12 && plazo <= 48;
  }

  return plazo >= 12 && plazo <= 72;
 });
...
const ajustarMonto = (valor: number) => {
 ...
 setMonto(
  aplicaLimiteJoven
   ? Math.min(montoMaximoJoven, montoNormalizado)
   : montoNormalizado,
 );
};
 ...
 setMonto(
 aplicaLimiteJoven
  ? Math.min(alternativa.monto, montoMaximoJoven)
  : alternativa.monto,
);
 ...
 if (
  ...
  (
   aplicaLimiteJoven &&
   resultado.monto > montoMaximoJoven
  )
 ) {
  return;
 }
 ...
 cuotaKivo: resultado.cuotaMensual,
 ...
 resultado.capacidad.porcentajeEndeudamiento,
 margenAhorroPorcentaje:
 resultado.capacidad.margenAhorroPorcentaje,
...
<div className="mt-6 grid gap-6 lg:grid-cols-[1fr_0.95fr]">
 ...
 value={monto}
 onValueChange={(value) => {
 if (value.floatValue === undefined) return;

 const nuevoMonto = normalizarMonto(
  value.floatValue,
 );

 setMonto(
  aplicaLimiteJoven
   ? Math.min(nuevoMonto, montoMaximoJoven)
   : nuevoMonto,
 );
}}
thousandSeparator="."
 ...
 {aplicaLimiteJoven ? (
  <>
   ...
   {formatBs(montoMaximoJoven)}.
  </>
 ) : (
  <>
   Puedes elegir desde{" "}
   {formatBs(REGLAS_SIMULACION.montoMinimo)}. Para montos
   superiores a {formatBs(REGLAS_SIMULACION.montoMaximo)},
   ingresa el valor directamente en el campo.
  </>
 )}
</p>
 ...
 value={Math.min(normalizarMonto(monto), montoMaximoSlider)}
 onChange={(event) => {
...
aria-label="Ajustar monto del préstamo"
 ...
 <span>{formatBs(montoMaximoSlider)}</span>
 </div>
 ...
 <dd className="font-bold text-white">
 {REGLAS_SIMULACION.tasaMensualPorcentaje.toFixed(2)}%
 </dd>
 </div>
 ...
 Tu nivel de endeudamiento está por debajo del 51%
 y puede pasar a la siguiente etapa de evaluación.
 </p>
 </div>
 </div>
 </motion.div>
 ...
 Con esta cuota, tu nivel de endeudamiento sería de{" "}
 {resultado.capacidad
  .porcentajeEndeudamiento
  .toFixed(2)}%.
 Kivo requiere que sea menor al 51%. Reduce el monto
